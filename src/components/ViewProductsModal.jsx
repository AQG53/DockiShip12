...
export default function ViewProductModal({ open, onClose, product }) {
  ...
  // Prefer full product detail for consistent shape (ProductVariant vs variants, pricing fields, etc.)
  const { data: detail, refetch } = useGetProduct(open && product?.id ? product.id : null, { refetchOnWindowFocus: false });
  ...
  const imagesByVariant = useMemo(() => {
    ...
    images.forEach(img => {
      ...
      // /uploads/<tenant>-<product>/<variant?>/file
  ...
  const hasVariants = useMemo(() => {
    ...
    // Guard: some APIs may echo parent as sole variant; treat that as simple
                    ...
                    <div className={card}>
                      ...
                      <div className="p-4 grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2">
                          ...
                          <p className={`${value} text-lg`}>{p.name || "—"}</p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className={value}>{p.originCountry || "—"}</p>
                        </div>

                        {/* Retail Price (Simple Product) */}
                        <div>
                          <p className={label}>Retail Price</p>
                          <p className={value}>{p?.retailPrice != null ? formatPrice(p.retailPrice) : "—"}</p>
                        </div>

                        {/* Stock (Simple Product) */}
                        <div>
                          <p className={label}>Stock on Hand</p>
                          <p className={value}>{p?.stock ?? p?.stockOnHand ?? "—"}</p>
                        </div>
                        ...
                        <div>
                          ...
                          <p className={value}>{formatDate(p.updatedAt)}</p>
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          {imagesByVariant.productLevel.length === 0 ? (
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              {imagesByVariant.productLevel.map((img) => (
                                <Thumb key={img.id} img={img} absImg={absImg} placeholder={IMG_PLACEHOLDER} onDelete={async () => {
                                  try { await deleteProductImage(p.id, img.id); await refetch?.(); } catch (e) { console.error(e); }
                                }} />
                              ))}
                            </div>
                          )}
                          ...
                          <div className="rounded-lg border border-gray-200 overflow-hidden">
                            ...
                            <div className="divide-y divide-gray-100">
                              {variantList.map((variant, idx) => {
                                ...
                                return (
                                  <div key={variant.id || idx} className="px-4 py-3 text-sm text-gray-800 hover:bg-gray-50 transition-colors">
                                    <div className="grid grid-cols-4 items-center">
                                      <div className="font-medium">{variant.sku || "—"}</div>
                                      <div>
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${variant.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-700"}`}>
                                    ...
                                    {/* Images inline for this variant */}
                                    <div className="mt-2">
                                      {imgs.length === 0 ? (
                                        <p className="text-xs text-gray-500">No images</p>
                                      ) : (
