...
import {
    useCourierMediums,
    useCreateCourierMedium,
    useUpdateCourierMedium,
    useDeleteCourierMedium,
} from "../hooks/useCourierMediums";
...
export default function CourierMediumsList() {
    ...
    const { data: rows = [], isLoading } = useCourierMediums({
        search: debouncedSearch,
        status: statusFilter.id,
    });
    const createMut = useCreateCourierMedium();
    const updateMut = useUpdateCourierMedium();
    const deleteMut = useDeleteCourierMedium();
    ...
    const openModal = (item = null) => {
        setEditing(item);
        setForm({ fullName: item?.fullName || "", shortName: item?.shortName || "" });
        setModalOpen(true);
    };
    ...
    const toolbar = (
        <>
            ...
            <Button variant="warning" onClick={() => openModal()}>
                <Plus size={16} className="mr-2" /> Add Courier
            </Button>
        </>
    );
    ...
    return (
        <div className="space-y-4">
            <DataTable
                ...
                rows={rows}
                isLoading={isLoading}
                gridCols="grid-cols-[1fr_1fr_100px_180px]"
                toolbar={toolbar}
            />
                ...
                <div className="space-y-3">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="e.g. Federal Express"
                            autoFocus
                        />
                    </div>
                    <div>
            ...
            <ConfirmModal
                ...
                title="Delete Courier Medium"
                loading={deleteMut.isPending}
            >
                Are you sure you want to delete <strong>{target?.fullName}</strong>? This action cannot be undone.
            </ConfirmModal>
        </div>
    );
}
