...
export default function WarehouseList() {
  ...
  const { data: rows = [], isLoading } = useWarehouses({ enabled: canRead });
    ...
    <div className="space-y-4">
      ...
      <div className={card}>
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200">
          <div>
            <p className="text-sm font-semibold text-gray-900">Warehouse listing</p>
            <p className="text-xs text-gray-500">Active locations and addresses</p>
          </div>
          {canManage && (
              ...
              >
                ...
                <div className="flex items-center justify-end gap-2">
                  {canManage && (
                    <>
                      <button
                        className="rounded-md border border-gray-300 px-2 py-1 text-[12px] hover:bg-gray-50"
      ...
      >
        ...
        <strong>{targetWarehouse?.name || "this warehouse"}</strong>? This will hide it from the active list.
      </ConfirmModal>
    </div>
  );
}
